...
import {getBoardComment, getBoardDetail, getBoardAddComment} from "../services/boardService"
...
const BoardDetailPage = () => {
  ...
  useEffect(() => console.log(newCommentText), [newCommentText])
  ...
  // 입력한 댓글 목록 불러오기 처리 함수
  const handleChange = (e) => {
    const { name, value } = e.target;

    // 현재 댓글 상태값 commentList를 업데이트
    setCommentList((prev) => ({
      ...prev,
      [name]: value,
    }));
  ...
  // preventDefault: 페이지 새로고침 방지
    ...
    // 서버에 댓글 등록 요청 보내는 함수
    getBoardAddComment({id, text: newCommentText})
      .then(() => {
    ...
    <div className='board_detail'>
      ...
      <ul>
        {commentList.map((comment, index) => {
          ...
          return <li key={safeKey}>{comment.text}</li>;
        })}
      </ul>
